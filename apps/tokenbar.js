...
export class TokenBar extends Application {
  ...
  /** @override */
  getData(options) {
    let css = [!game.user.isGM ? 'hidectrl' : null].filter((c) => !!c).join(' ');
    let pos = this.getPos();
    return {
      tokens: this.tokens,
  ...
  async mapToken(token) {
    ...
    let img =
      setting('token-pictures') == 'actor' && token.actor != undefined
  ...
  getResourceBar(token, bar) {
    ...
    if (token.data.displayBars > 0) {
      ...
      if (attr != undefined && attr.type == 'bar') {
        ...
        if (val != undefined) {
          let color =
            bar === 'bar1' ? [1 - pct / 2, pct, 0] : [0.5 * pct, 0.7 * pct, 0.5 + pct / 2];
          resource = {
            value: val,
            pct: pct * 100,
            color:
  ...
  async updateToken(tkn) {
    ...
    if (tkn?.resource2?.value != tkn.token.getBarAttribute('bar2')?.value) {
      //getAttrProperty(tkn.token.actor.data.data, tkn.token.data.bar2.attribute)) {
  ...
  activateListeners(html) {
    ...
    html
      .find('.token')
      .click(this._onClickToken.bind(this))
      .dblclick(this._onDblClickToken.bind(this))
      .hover(this._onHoverToken.bind(this));
    ...
    let context = new ContextMenu(html, '.token', [
      {
        ...
        condition: (li) => {
          ...
          let players = game.users.entities.filter(
            (u) =>
              !u.isGM &&
              (entry.token.actor.data.permission[u.id] == 3 ||
                entry.token.actor.data.permission.default == 3)
          );
